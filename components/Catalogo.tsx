...
function aplicarBusquedaTexto(query: any, termino: string, columnas: string[], incluirNormalizada: boolean) {
  ...
  if (palabras.length === 1) {
    const alternativas = columnas.map(c => `${c}.ilike.%${limpio}%`)
  ...
  palabras.forEach(palabra => {
    q = q.or(columnas.map(c => `${c}.ilike.%${palabra}%`).join(','))
  })
  return q
}
              ...
              <div className="flex items-center justify-between w-full md:w-auto gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <Link href="/radiacor" className={`flex items-center gap-3 transition-opacity ${modo === 'radiacor' ? '' : 'opacity-60 hover:opacity-100'}`}>
                    <span className="w-11 h-11 md:h-14 md:w-14 border-2 border-white flex items-center justify-center text-white font-black text-xl md:text-2xl shrink-0">R</span>
                    <div className="hidden md:flex flex-col justify-center">
